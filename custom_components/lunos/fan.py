...
class LUNOSFan(FanEntity):
    ...
    def __init__(
        ...
        relay_w1,
        relay_w2,
        default_preset: str = DEFAULT_VENT_MODE,
    ):
        """Init this sensor."""
    ...
    def _init_fan_speeds(self, model_config: dict) -> None:
    ...
    def _init_vent_modes(self, model_config: dict) -> None:
    ...
    def _init_presets(self, model_config: dict, default_preset: str) -> None:
        ...
        if default_preset not in self._preset_modes:
            LOG.warning(
                f'Default preset {default_preset} is not valid: {self._preset_modes}'
            )
    ...
    @callback
    @callback
    def _detected_relay_state_change(self, event) -> None:
        ...
        if to_state != from_state:
            LOG.info(
                f"{entity} changed: {from_state} -> {to_state}, updating '{self._name}'"
            )
    ...
    async def async_set_ventilation_mode(self, vent_mode: str) -> None:
        ...
        if vent_mode == VENT_SUMMER:
            ...
            LOG.error(
                "Ventilation mode '{vent_mode}' not supported: {self._vent_modes}"
            )
            return
    ...
    def _determine_current_relay_speed(self) -> str | None:
        ...
        LOG.info(
            f'Could not determine current speed from W1/W2 relays: {current_state}'
        )
        return None

    def _update_speed(self, speed: str) -> None:
